...
if __name__ == '__main__':
    ...
    identification = api.identify(
        ...
        details=[
            ...
            'url',              # string - localized - link to mushroom profile page (usually Wikipedia)
            'description',      # string - localized, licensed - short description from Wikipedia
            'edibility',        # string - one of choice: highly regarded for its edibility and widely used in cooking
            # edible: safe to eat
            # edible when cooked: only edible when cooked caution: there are warnings regarding the mushroom’s
            #       edibility, such as stomach irritations or easy confusion with poisonous mushroom species
            ...
            'characteristic',   # dict - possible values are the following: hymenium attachment, hymenium type,
            # mushroom cap shape, mushroom ecological type, spore print color, stipe character
            'look_alike',       # list of dicts - mushrooms that are frequently confused with the suggested species;
            # each taxon contains entity_id, name and url
